...
├─ crates/
│  ├─ app-contracts/            # App リポジトリ内で共有する Rust ホスト契約
│  ├─ app-domain/               # App 自身が所有するドメイン / ランタイム実装
│  ├─ app-host-core/            # Web / Tauri 共有の bootstrap/runtime ビルダー
...
このワークスペースは現在 `App` と `Game` の 2 つのプロダクトリポジトリだけを保持しています。App 専用ドメインは `crates/app-domain` に置き、Rust -> TypeScript 契約同期もこのリポジトリ内で完結します。
